...
{
	Adina::GLTexture texture = Adina::ResourceManager::getTexture("Assets/blue_ninja.png");
	m_color = color;
	m_drawDims = drawDims;
...
void Player::draw(Adina::SpriteBatch& spriteBatch) {
	glm::vec4 destRect;
...
void Player::drawDebug(Adina::DebugRenderer& debugRenderer) {
	m_capsule.drawDebug(debugRenderer);
}

void Player::update(Adina::InputManager& inputManager)
{
	b2Body* body = m_capsule.getBody();
